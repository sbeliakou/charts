...
To install the chart with the release name `jenkins-release`:

```bash
$ helm install --name jenkins-release incubator/jenkins
```
...
1. Install the chart
```bash
$ helm install --name jenkins-release --set Persistence.ExistingClaim=PVC_NAME stable/jenkins
```
